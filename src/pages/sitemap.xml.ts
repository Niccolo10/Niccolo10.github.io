...
export async function GET() {
  ...
  const cases = await getCollection('bugCode', ({data}) => !data.draft);
  const paths = ['/', '/about/', '/research/', '/archive/', '/bug-code/', ...articles.map(e => `/research/${e.id}/`), ...cases.map(e => `/bug-code/${e.id}/`)];
