...
Casebook reading order groups articles by chapter, then uses optional numeric `order` frontmatter within each chapter, lower first, with a default of 50. Article numbers and URLs stay stable. The invitation article uses 900 to close the identity chapter. Maintain chapter assignments and short vulnerability labels in `src/lib/casebook-order.ts`; unmapped articles appear in Further cases. Use its comparator consistently for casebook navigation. Each article links to one next article, and the last returns to contents.
