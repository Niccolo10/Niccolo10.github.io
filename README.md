...
The main site includes the nine public case studies under `/bug-code/<slug>/`, the research index, RSS, and sitemap. Reading order: template injection, internal path manipulation, SAML XXE, account identity confusion, future-price manipulation, cloud API authentication, wildcard identifier disclosure, password-cookie storage, invitation hijacking. Development mode also displays any unpublished local drafts not already represented by a public article. The `/drafts/` workbench remains local only. Production checks assert public article presence and exclude unpublished private slugs and editorial notices.
