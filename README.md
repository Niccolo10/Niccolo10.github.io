...
Nine reviewed articles are published. `node scripts/check-editorial-examples.mjs` validates HTTP/JSON examples in the published content and any local editorial copies, and executes the cookie encoding and URL construction demonstrations locally.

Casebook reading order groups articles by chapter, then uses optional numeric `order` frontmatter within each chapter, lower first, with a default of 50. Article numbers follow the chapter reading sequence; URLs stay stable. Update frontmatter and corollary numbers together when renumbering. The invitation article uses 900 to close the identity chapter. Maintain slug-keyed chapter assignments and short vulnerability labels in `src/lib/casebook-order.ts`; unmapped articles appear in Further cases. Use its comparator consistently for casebook navigation. Each article links to one next article, and the last returns to contents.
...
The nine reviewed Bug Code articles are published from `src/content/bug-code/`, with explicit `draft: false` and a publication `date`. Edit these public copies for subsequent releases. Private source reports and editorial provenance remain outside Git. Publication was authorized by the site owner for these versions; this is not a claim of program approval. New articles remain draft by default. The Python model checker also executes the SQL wildcard example against an in-memory database.
...
The main site includes the nine public case studies under `/bug-code/<slug>/`, the research index, RSS, and sitemap. Chapters: Injection and server-side behavior (template injection, internal path manipulation, SAML XXE); Identity and access (account identity confusion, cloud API authorization, invitation hijacking); Business logic and data exposure (future-price manipulation, wildcard identifier disclosure, password-cookie storage). Development mode also displays any unpublished local drafts not already represented by a public article. The `/drafts/` workbench remains local only. Production checks assert public article presence and exclude unpublished private slugs and editorial notices.
